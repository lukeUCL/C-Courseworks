...
open(FOO, "sh0019.c") || die "Did you delete sh0019.c?";
$lines = 0;
...
# Conditionals
    ...
    [ 'Test COND3',
      'grep -cv NotThere ../sh0019.c && echo Wanted',
      "$lines Wanted" ],

    [ 'Test COND4',
      'grep -c NotThere ../sh0019.c && echo Unwanted',
      '0' ],
...
# Pipelines
    ...
    [ 'Test PIPE5',
      '../sh0019 -q cmd%%.sh; ps | grep sleep | grep -v grep | head -n 1 | wc -l',
      'Hello Bye 1',
      CMD_INIT => 'echo "echo Hello; sleep 2 & echo Bye" > cmd%%.sh'],
    ...
    [ 'Test PIPE13',
      ...
      'Hello 1',
      CMD_INIT => 'echo "echo Hello; sleep 0.4" > cmd%%.sh',
      CMD_CLEANUP => 'echo "sleep 0.2 ; ps T | grep sleep | grep -v grep | head -n 1 | wc -l" > cmd%%b.sh; ../sh0019 -q cmd%%b.sh',
...
# Redirection
    ...
    [ 'Test REDIR5',
      'tr hb HB < f%%.txt | sort | ../sh0019 -q cmd%%.sh',
...
# Fancy conditionals
    ...
    ,[ 'Test ADVBGCOND1', # Background conditionals
       'sleep 0.2 && echo second & echo first',
       'first second',
       CMD_CLEANUP => 'sleep 0.2' ]

    ,[ 'Test ADVBGCOND2', # Background conditionals
...
# check for a ton of existing sh0019 processes
$me = `id -un`;
chomp $me;
open(SH0019, "ps uxww | grep '^$me.*sh0019' | grep -v grep |");
$nsh0019 = 0;
$nsh0019 += 1 while (defined($_ = <SH0019>));
close SH0019;
if ($nsh0019 > 5) {
    print STDERR "\n";
    print STDERR "${Red}**** Looks like $nsh0019 ./sh0019 processes are already running.\n";
    print STDERR "**** Do you want all those processes?\n";
    print STDERR "**** Run 'killall -9 sh0019' to kill them.${Off}\n\n";
}
...
sub run_sh0019 ($;%) {
    ...
    # ensure that we ignore SIGINT, but only for INTR tests
    if (exists($opt{"int_delay"})) {
	$saveintact = $SIG{"INT"};
	$SIG{"INT"} = "IGNORE";
    }
    ...
    unless (POSIX::setpgid($run0019_pid, $run0019_pid)) {
        if (!$!{EACCES}) {
            die("setpgid: $!\n");
        }
    }

    POSIX::tcsetpgrp(fileno(TTY), $run0019_pid) or die "tcsetpgrp: $!\n";
    ...
    eval {
        do {
            ...
            if (waitpid($run0019_pid, WNOHANG) > 0) {
                $answer->{"status"} = $?;
                die "!";
            }
            ...
            if (defined($size_limit) && $size_limit_file && @$size_limit_file) {
                my($len) = 0;
                $out = run_sh0019_pipe($out, fileno(OR), $size_limit);
    ...
    $answer->{"output"} = run_sh0019_pipe($out, fileno(OR), $size_limit);
    close(OR);
...
sub run (@) {
    ...
    my($info) = run_sh0019($cmd, "stdin" => $stdin, "stdout" => $outfile, "time_limit" => 10, "size_limit" => 1000, "dir" => "out", "nokill" => 1, "delay" => 0.05, "int_delay" => $opts{CMD_INT_DELAY});

    if ($opts{CMD_CLEANUP}) {
        if ($opts{CMD_CAREFUL_CLEANUP}) {
            my($infox) = run_sh0019("{ " . $opts{CMD_CLEANUP} . "; } >>$outfile 2>&1", "time_limit" => 5, "dir" => "out", "stdin" => "/dev/stdin", "stdout" => "/dev/stdout");
